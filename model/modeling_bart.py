...
class BartEncoder(BartPretrainedModel):
    ...
    def forward(
        ...
        ####################
        # print("EMBED POS SHAPE =", embed_pos.size())
        # print("INPUTS EMBEDS SHAPE =", inputs_embeds.size())
        ####################
...
class BartForConditionalGeneration(BartPretrainedModel):
    ...
    def __init__(self, config: BartConfig):
        ...
        # Adding a classification head to be able to choose between candidates
    ...
    def forward(
        ...
        ####################################
        # Avoiding some size mismatch issues
        ...
        # Model forward, outputs == Seq2SeqModelOutput
        outputs = self.model(
            input_ids,
            attention_mask=attention_mask,
        ...
        if cls_index is not None:
            ...
            # Taking the last hidden state as the sentence embedding
            sentence_representation = outputs.last_hidden_state[cls_mask, :]\
                .view(
            ...
            cls_logits = cls_logits.view(batch_size, -1)

        # Classification loss
        cls_loss = None
        if clslabel is not None:
            ce_loss_fct = CrossEntropyLoss()
            cls_loss = ce_loss_fct(cls_logits, clslabel.view(-1))
